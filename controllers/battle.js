...
exports.sweep = exports.winBattle = exports.giveUpBattle = exports.checkBeforeBattle = exports.startBattle = void 0;
const bcrypt_1 = __importDefault(require("bcrypt"));
const models_1 = require("../models");
const common_1 = require("./common");
const getPigPoint = (arrangement, mysteriousCreatureEachUpgrade, monarchEachUpgrade) => {
    let pigPoint = 0;
...
const getGreedNumber = (arrangement) => {
    let greedNumber = 0;
    const arrangementInfoArr = arrangement.split('/');
    for (const arrangementInfo of arrangementInfoArr) {
        if (arrangementInfo === "0") {
...
const startBattle = async (req, res, next) => {
    try {
        ...
        if (!Number.isInteger(currentBattleGradeGroupIndex) ||
...
const checkBeforeBattle = async (req, res, next) => {
    try {
...
exports.checkBeforeBattle = checkBeforeBattle;
const giveUpBattle = async (req, res, next) => {
    try {
        const user = res.locals.user;
...
exports.giveUpBattle = giveUpBattle;
const winBattle = async (req, res, next) => {
    try {
        const user = res.locals.user;
        user.currentBattleOrNot = false;
        await user.save();
        ...
        for (const moveInfo of moveLog) {
            ...
            if (!Number.isInteger(proceedTick) ||
                proceedTick < 1 ||
                proceedTick > 3600) {
                const errorObj = {
                    place: "controllers-battle-winBattle",
            ...
            if (!validZoneNumberList.includes(originZoneNumber) ||
                ...
                if ([0, 5].includes(this.typeNumber)) {
                    ...
                    skillTargetEnemies.forEach((skillTarget) => {
                        skillTarget.getDamage(this.typeNumber === 0
                            ? Math.round((((this.totalAttackNumbers % 20) + 1) / 10) *
                                skillCoefficientArr[this.grade] *
                                this.attackDamage *
                                0.2)
                            : Math.round(skillCoefficientArr[this.grade] * this.attackDamage * 0.2));
                    });
                    if (this.grade < 4)
                        return;
                    ...
                    skillTargetEnemies.forEach((enemy) => {
                        enemy.getDamage(Math.round((this.typeNumber === 2
                            ? 0.4
                            : this.typeNumber === 4
                                ? 0.5
                                : 0.1) *
        ...
        const highestBattleGradeGroupIndex = Math.floor((highestBattleGrade + 99) / 100);
        ...
        const { dailyBattleWinRewardOrNot, dailyBattleWinCounter, weeklyBattleWinRewardOrNot, weeklyBattleWinCounter, } = achivement;
...
const sweep = async (req, res, next) => {
    try {
        ...
        const achivement = await user.getAchivement();
        if (currentBattleGradeGroupIndex ===
