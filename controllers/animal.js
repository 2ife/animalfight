...
exports.upgradeAnimal = exports.combineAnimal = exports.summonAnimal = exports.saveArrangement = void 0;
...
exports.saveArrangement = saveArrangement;
const summonAnimal = async (req, res, next) => {
    try {
        const user = res.locals.user;
        ...
        const allBabiesRewardOrNot = Boolean(Number(allAnimalsRewardOrNotInfoArr[0]));
        ...
        for (const animalTypeNumber in newAnimals) {
            ...
            babiesInfoArr[animalTypeNumber] = `${Number(babiesInfoArr[animalTypeNumber]) + animalAmounts}`;
            if (!allBabiesRewardOrNot) {
                const targetBabyGetOrNot = Boolean(Number(allBabiesGetOrNotInfoArr[animalTypeNumber]));
...
exports.summonAnimal = summonAnimal;
const combineAnimal = async (req, res, next) => {
    try {
        const user = res.locals.user;
        ...
        const animalsInfo = await user.getAnimalsInfo();
        const achivement = await user.getAchivement();
        const { allSmallsGetOrNot, allBeastsGetOrNot, allMysteriousCreaturesGetOrNot, allMonarchsGetOrNot, allAnimalsRewardOrNot, } = achivement;
        const combineInfoList = getAnimalCombineInfoList(targetAnimalGrade, targetAnimalTypeNumber, animalsInfo);
        ...
        for (const combineInfo of combineInfoList) {
            ...
            targetGradeAnimalAmountList[typeNumber] = `${Number(targetGradeAnimalAmountList[typeNumber]) - neededAmounts}`;
            switch (grade) {
                case 0: {
        ...
        newAnimalGradeAnimalAmountList[targetAnimalTypeNumber] = `${Number(newAnimalGradeAnimalAmountList[targetAnimalTypeNumber]) + 1}`;
        ...
        const allAnimalsRewardOrNotInfoArr = allAnimalsRewardOrNot.split("/");
        const targetGradeAllAnimalsRewardOrNot = Boolean(Number(allAnimalsRewardOrNotInfoArr[newAnimalGrade]));
        ...
        if (!targetGradeAllAnimalsRewardOrNot) {
            ...
            const targetGradeAllAnimalsGetOrNotInfoArr = targetGradeAllAnimalsGetOrNot.split("/");
            const targetAnimalGetOrNot = Boolean(Number(targetGradeAllAnimalsGetOrNotInfoArr[targetAnimalTypeNumber]));
...
exports.combineAnimal = combineAnimal;
const upgradeAnimal = async (req, res, next) => {
    try {
        const user = res.locals.user;
        ...
        if ((upgradePart === "grade" && ![0, 1, 2, 3, 4].includes(upgradeIndex)) ||
        ...
        const targetUpgrade = upgradePart === "grade"
            ? gradeUpgrade
            : upgradePart === "mysteriousCreature"
                ? mysteriousCreatureEachUpgrade
                : monarchEachUpgrade;
        const targetUpgradeInfoArr = targetUpgrade.split("/");
        const targetUpgradeCurrentValue = Number(targetUpgradeInfoArr[upgradeIndex]);
        const targetUpgradeMaxValue = upgradePart === "grade"
            ? 2 ** upgradeIndex * 20
            : upgradePart === "mysteriousCreature"
                ? 25
                : 50;
        if (((upgradePart === "grade" && upgradeIndex < 4) ||
            upgradePart !== "grade") &&
        ...
        const neededGold = upgradePart === "grade"
...
exports.upgradeAnimal = upgradeAnimal;
