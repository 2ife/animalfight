...
const clickBattleGrade = (event) => {
    ...
    battleRewardAmountContainers.forEach((amountContainer, index) => {
        if (index >= 2)
            return;
        amountContainer.innerText = formatNumber([currentGrade * 10 * Math.ceil(currentGrade / 20), currentGrade * 20 * Math.ceil(currentGrade / 20)][index]);
    });
...
const putMyMails = async () => {
    try {
        ...
        else if (menuBtns[6].style.color === "blue" && myMailData.length === 0) {
